...
public:
    void run(bool use_wireframe) {
        this->wireframe = use_wireframe;
        initWindow();
        initVulkan();
        mainLoop();
        cleanup();
...
private:
    ...
    // Vulkan setup
    // could be named createVulkanInstance()
    void createInstance() {
        // misc app info
    ...
    // Will only be called if debugging is on
    // determines that each validation layer requested is actually there
    ...
    // Since Vulkan is a GPU API, we can't just "debug" the normal way
    // We have to set up some sort of program/service that will help us
    // The Vulkan spec has explicit structures to help us debug
    // but they need to be allocated and given a callback function to call
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.ambient = glm::vec3(0.1, 0.1, 0.0);
...
/*
    ...
    void initVulkan() {
        ...
        createIndexBuffer();
        createUniformBuffers();
        createDescriptorPool(); // binds uniforms
        createDescriptorSets();
        createCommandBuffers();
        createSyncObjects();
    }
    ...
    void cleanup() {
        ...
        // notice that, generally, the order that objects are destroyed
        // is the opposite of the order they were created in
        ...
        // we don't need to destroy the physical device 
        // since destroying the instance imlpicitly does that for us
        // but this call destroys the *logical* device
        vkDestroyDevice(device, nullptr);

        vkDestroySurfaceKHR(instance, surface, nullptr); // window surface

        // this call needs to happen AFTER the logical device is destroyed
        // since destroying the instance means the device is no longer allocated
        // so doing it in the other order will create a segfault
